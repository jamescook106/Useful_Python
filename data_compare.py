...
for i in input_name_array:
	serial_master.append(np.genfromtxt(serial_directory + '/' +i, dtype=np.complex128, converters={0: lambda s: complex(s.decode().replace('+-', '-'))}))
	parallel_master.append(np.genfromtxt(parallel_directory + '/' +i, dtype=np.complex128, converters={0: lambda s: complex(s.decode().replace('+-', '-'))}))
