...
## This is a basic supernovae parameter estimation MCMC Code
## It was created as part of the ICIC Data Analysis Workshop 2016
###############################################
...
def mu(f_mu_h, f_mu_z, f_mu_omega_matter):
    ...
    f_mu_eta = (
        ...
        * pow(
            (
                ((1.0) / (pow(f_mu_a, 4.0)))
                - 0.1540 * ((f_mu_s) / (pow(f_mu_a, 3.0)))
                + 0.4304 * pow(((f_mu_s) / (f_mu_a)), 2)
                + 0.19097 * ((pow(f_mu_s, 3)) / (f_mu_a))
                + 0.066941 * pow(f_mu_s, 4)
            ),
            ((-1.0) / (8.0)),
        )
    )
...
def likelihood(f_l_h, f_l_omega_matter, f_l_inverted_matrix, f_l_z_data, f_l_mu_data):
    ...
    for like_q in range(0, 31):
        f_l_calc[like_q] = f_l_mu_data[like_q] - mu(
            f_l_h, f_l_z_data[like_q], f_l_omega_matter
        )

    f_l_result = (-0.5 * np.dot(f_l_calc, np.dot(f_l_inverted_matrix, f_l_calc))) - (
        pow((f_l_h - 0.738), 2.0) / (2 * 0.024 * 0.024)
    )  # Second term is from Hubble
...
def MCMC(f_MCMC_N, f_MCMC_var, f_MCMC_inverted_matrix, f_MCMC_z_data, f_MCMC_mu_data):
    ...
    for MCMC_MAIN in range(0, f_MCMC_N):
        ...
        MCMC_1 = likelihood(
            f_MCMC_h1, f_MCMC_o1, f_MCMC_inverted_matrix, f_MCMC_z_data, f_MCMC_mu_data
        )
...
def Gelman_Rubin(f_G_R):
    ...
    BO = ((len(f_G_R[0][0])) / (len(f_G_R) - 1.0)) * np.sum(Boa)
    BH = ((len(f_G_R[0][1])) / (len(f_G_R) - 1.0)) * np.sum(Bha)

    WH = np.mean(total_chain_var_h)
    WO = np.mean(total_chain_var_o)

    VH = ((len(f_G_R[0][0]) - 1.0) / (len(f_G_R[0][0]))) * WH + (
        (len(f_G_R) + 1.0) / (len(f_G_R) * (len(f_G_R[0][0])))
    ) * BH
    VO = ((len(f_G_R[0][0]) - 1.0) / (len(f_G_R[0][0]))) * WO + (
        (len(f_G_R) + 1.0) / (len(f_G_R) * (len(f_G_R[0][0])))
    ) * BO
...
input_data = np.genfromtxt(
    "jla_mub.txt", delimiter=" ", skip_header=0, skip_footer=0, names=["z", "mu"]
)

print("Data imported")

# Load the Covariant Matrix
input_covariant_matrix = np.genfromtxt(
    "jla_mub_covmatrix.txt", delimiter=" ", skip_header=0, skip_footer=0
)
...
if args.con:
    ...
    GR.append(
        MCMC(
            ...
            0.7,
            0.3,
            inverted_input_covariant_matrix,
            input_data["z"],
            input_data["mu"],
        )
    )
